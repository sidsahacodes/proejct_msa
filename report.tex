...
Variables with more than 20\% missingness were removed prior to delivery; remaining numeric missing values were median-imputed. All predictors were standardized to zero mean and unit variance before PCA and factor analysis, because the variables represent different measurement types and retain unequal variance after normalization. For the supervised models, the 70/30 stratified train-test split (1{,}395 train / 599 test, \texttt{random\_state\,=\,42}) is performed on the raw predictor matrix \emph{before} fitting the scaler, PCA, and factor analysis; those transforms are estimated on the training rows only and applied to the held-out test rows, so test-set statistics never inform any feature-engineering step.
...
Correlation PCA was fit on all 100 standardized predictors. Figure~\ref{fig:pca}(a) shows the scree plot: PC1 alone explains 25.2\% of total variance, and the first 11 components together explain 80\%. The scree elbow and the 80\% threshold both support retaining 11--13 components for downstream use. PC2 explains 16.8\%.
...
Table~\ref{tab:classification} reports performance on the held-out test set. ROC-AUC ranges from 0.906 to 0.913 across the five unpenalized models. The logistic model on original predictors achieves accuracy 0.846; the logistic models on PCA and factor scores tie for the highest AUC (0.913) with only 11 and 7 inputs, respectively. Notably, dimension reduction to 11 PCA scores or 7 factor scores does not degrade predictive accuracy relative to the full 100-predictor model, indicating that the predictive signal is well-captured by the leading components. LDA performs marginally below logistic regression, which is consistent with LDA's equal-covariance assumption being only approximately satisfied.

\subsection{Regularization}

Ridge, lasso, and elastic-net logistic regression were fit on the full 100-predictor matrix with regularization strength selected by 5-fold cross-validated ROC-AUC. Ridge and elastic net tie for the best overall AUC (0.915). Lasso attains AUC~0.913 while retaining only 47 of 100 predictors, and elastic net retains 44. The selected lasso predictors include investment income, family-structure variables, housing vacancy, residential mobility, and law-enforcement indicators. The gains over unpenalized logistic regression are modest, suggesting the predictive signal is strong enough that regularization improves stability more than accuracy. Rows for Ridge, Lasso, and Elastic Net appear in Table~\ref{tab:classification} alongside the unpenalized models.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccc}
    ...
    Logistic (PCA)       & 0.831 & 0.829 & 0.834 & \textbf{0.913} \\
    Logistic (Factors)   & 0.833 & 0.829 & 0.837 & \textbf{0.913} \\
    LDA (PCA)            & 0.825 & 0.802 & 0.847 & 0.906 \\
    LDA (Factors)        & 0.825 & 0.802 & 0.847 & 0.906 \\
    \midrule
    Ridge (100 coef.)    & 0.843 & 0.826 & 0.860 & \textbf{0.915} \\
    Lasso (47 coef.)     & 0.848 & 0.822 & 0.874 & 0.913 \\
    Elastic Net (44 coef.)& 0.838 & 0.819 & 0.857 & \textbf{0.915} \\
    \bottomrule
  \end{tabular}
\end{table}
...
Prediction performance is consistent across all input representations: 11 PCA scores, 7 factor scores, and 100 original predictors all yield ROC-AUC in the 0.906--0.915 range. This suggests that the predictive content of the full predictor set is almost entirely contained in the few leading dimensions of variation—the same dimensions identified by PCA and factor analysis as substantively meaningful. Regularization offers marginal additional gains; the main benefit of lasso is parsimonious variable selection, not improved discrimination.
